...
(function (Combinator) {
     ...
     * it until we see it.
     */
    function matcher(left, combinator, right, element, context = Context.empty()) {
        let traversal = Node.Traversal.empty;
        let rightMatches = false;
        if (Slotted.isSlotted(right) ||
     ...
     */
    const ancestorMatchesLeft = Cache.memoize(function (element, context, left) {
        return element
            .parent()
            .filter(isElement)
            .some((parent) => left.matches(parent, context) ||
     ...
     * @internal
     */
    Combinator.parse = either(delimited(option(Token.parseWhitespace), either(map(Token.parseDelim(">"), () => Combinator.DirectDescendant), map(Token.parseDelim("~"), () => Combinator.Sibling), map(Token.parseDelim("+"), () => Combinator.DirectSibling))), map(Token.parseWhitespace, () => Combinator.Descendant));
})(Combinator || (Combinator = {}));

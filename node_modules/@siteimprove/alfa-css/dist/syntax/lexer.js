 ...
 */
const isDigit = (code) => code >= CHAR_ZERO && code <= CHAR_NINE;
/**
 ...
 */
const isUppercaseLetter = (code) => code >= CHAR_UPPER_A && code <= CHAR_UPPER_Z;
/**
 * {@link https://drafts.csswg.org/css-syntax/#lowercase-letter}
 */
const isLowercaseLetter = (code) => code >= CHAR_LOWER_A && code <= CHAR_LOWER_Z;
/**
 ...
 */
const isNonAscii = (code) => code >= CHAR_NON_ASCII;
/**
 * {@link https://drafts.csswg.org/css-syntax/#newline}
 */
const isNewline = (code) => code === CHAR_NEWLINE;
/**
 * {@link https://drafts.csswg.org/css-syntax/#whitespace}
 */
const isWhitespace = (code) => isNewline(code) || code === CHAR_TAB || code === CHAR_SPACE;
/**
 * {@link https://drafts.csswg.org/css-syntax/#non-printable-code-point}
 */
const isNonPrintable = (code) => (code >= CHAR_NULL && code <= CHAR_BACKSPACE) ||
 ...
 */
const isNameStart = (code) => isLetter(code) || isNonAscii(code) || code === CHAR_UNDERSCORE;
/**
 * {@link https://drafts.csswg.org/css-syntax/#name-code-point}
 */
const isName = (code) => isNameStart(code) || isDigit(code) || code === CHAR_MINUS;
/**
 * {@link https://infra.spec.whatwg.org/#surrogate}
 */
const isSurrogate = (code) => code >= CHAR_SURROGATE_MIN && code <= CHAR_SURROGATE_MAX;
/**
 * {@link https://drafts.csswg.org/css-syntax/#starts-with-a-valid-escape}
 */
const startsValidEscape = ([input, i]) => input.charCodeAt(i) === CHAR_BACKSLASH && !isNewline(input.charCodeAt(i + 1));
/**
...
const consumeToken = ([input, i,]) => {
    ...
    while (i < n) {
        ...
        if (first === CHAR_SLASH && input.charCodeAt(i + 1) === CHAR_ASTERISK) {
            i += 2;
            while (i < n) {
                if (input.charCodeAt(i) === CHAR_ASTERISK &&
    ...
    switch (code) {
        ...
        case CHAR_MINUS:
            ...
            if (input.charCodeAt(i + 1) === CHAR_MINUS &&
        ...
        case CHAR_DOT:
            ...
            return [[input, i + 1], Token.delim(code)];
        case CHAR_LESS_THAN:
            if (input.charCodeAt(i + 1) === CHAR_EXCLAMATION &&

 ...
 * @public
 */
export class CSS4Color extends Value {
    static of(spaceOrColor, coordinates, alpha = 1) {
        if (typeof spaceOrColor === "string") {
            ...
            try {
                const color = coordinates === undefined
    ...
    constructor(color) {
        super("color", false);
        this._color = color;
        this._roundedCoordinates = color.coords.map(round);
        this._srgb = color.to("srgb");
        this._red = Percentage.of(this._srgb.r ?? 0);
        this._green = Percentage.of(this._srgb.g ?? 0);
        this._blue = Percentage.of(this._srgb.b ?? 0);
    ...
    toJSON() {
        return {
            ...
            coordinates: this._color.coords.map((c) => c === null ? null : Number.of(c).value),
            sRGB: Array.toJSON([this._red, this._green, this._blue]),
 ...
 */
(function (CSS4Color) {
    function isCSS4Color(value) {
        return value instanceof CSS4Color;
    }
    CSS4Color.isCSS4Color = isCSS4Color;
    /*
     ...
     */
    function catcher(parser, stringifier) {
        return mapResult(parser, (parsed) => {
            try {
                return Result.of(new Color(stringifier(parsed)));
            }
            catch (e) {
    ...
    const parseHash = catcher(Token.parseHash(), (hash) => `${hash}`);
    const parseIdent = catcher(Token.parseIdent(), (ident) => ident.value);
    /*
     ...
     */
    const parseComponent = either(map(either(Number.parse, Percentage.parse, Angle.parse), (component) => component.resolve().toString()), map(Token.parseFirst, (token) => token.toString()));
    const parseFunction = catcher(Function.parse(undefined, zeroOrMore(parseComponent)), ([func, body]) => `${func.name}(${body.join("")})`);
    const parseColor = either(parseHash, parseIdent, parseFunction);
    CSS4Color.parse = map(parseColor, CSS4Color.of);
})(CSS4Color || (CSS4Color = {}));
const internalPrecision = 5;
function round(value, nullReplacement) {
...
//# sourceMappingURL=css4-color.js.map
 ...
 * @public
 */
export function hasSoftWrapOpportunity(device, context) {
    return or(
    // We start with the most likely to succeed.
    (text) => String.hasSoftWrapOpportunity(text.data), hasHyphenationOpportunity(device, context), hasComputedStyle("word-break", ({ value }) => value === "break-all", device, context), hasComputedStyle("line-break", ({ value }) => value === "anywhere", device, context), hasComputedStyle("overflow-wrap", ({ value }) => value === "break-word" || value === "anywhere", device, context));
}
//# sourceMappingURL=has-soft-wrap-opportunity.js.map
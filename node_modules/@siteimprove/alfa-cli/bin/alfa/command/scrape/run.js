...
import { Archive, Awaiter, Credentials, Scraper, Screenshot, } from "@siteimprove/alfa-scraper";
import { Timeout } from "@siteimprove/alfa-time";
import { URL } from "@siteimprove/alfa-url";
export const run = async ({ flags, args: { url: target }, }) => {
    ...
    const orientation = flags.orientation === "portrait"
        ? Viewport.Orientation.Portrait
        : Viewport.Orientation.Landscape;
    const device = Device.of(Device.Type.Screen, Viewport.of(flags.width, flags.height, orientation), Display.of(flags.resolution), Scripting.of(flags.scripting));
    const credentials = flags.username.isSome() && flags.password.isSome()
    ...
    for (const path of flags.screenshot) {
        switch (flags.screenshotType) {
            case "png":
                screenshot = Screenshot.of(path, Screenshot.Type.PNG.of(flags.screenshotBackground));
                break;
            case "jpeg":
                screenshot = Screenshot.of(path, Screenshot.Type.JPEG.of(flags.screenshotQuality));
        }
    }
    let archive;
    ...
    for (const path of flags.headersPath) {
        try {
            const parsed = Array.from(JSON.parse(fs.readFileSync(path, "utf-8")));
            headers.push(...parsed.map((header) => Header.of(header.name, header.value)));
        }
        catch (err) {
    ...
    for (const path of flags.cookiesPath) {
        try {
            const parsed = Array.from(JSON.parse(fs.readFileSync(path, "utf-8")));
            cookies.push(...parsed.map((cookie) => Cookie.of(cookie.name, cookie.value)));
        }
        catch (err) {
            if (err instanceof Error) {
                return Err.of(err.message);
    ...
    const result = await scraper.scrape(URL.parse(target, url.pathToFileURL(process.cwd() + path.sep).href).getUnsafe(), {

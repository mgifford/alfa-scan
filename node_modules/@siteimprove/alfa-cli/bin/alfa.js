...
const { argv: [, bin], platform, arch, version, } = process;
const application = Command.withSubcommands(path.basename(bin), `${pkg.name}/${pkg.version} ${platform}-${arch} node-${version}`, `The tool for all your accessibility needs on the command line.`, {
...
application
    ...
    .catch((err) => Err.of(`${err.stack ?? err.message}`))
    .then(async (result) => {

...
const { hasComputedStyle, hasInitialComputedStyle, isPositioned, isFlexOrGridChild, } = Style;
/**
...
export function createsStackingContext(device) {
    ...
    hasComputedStyle("will-change", and((List.isList), (list) => list.some((ident) => ident.is("position", "z-index", "opacity", "mix-blend-mode", "transform", "scale", "rotate", "translate", "filter", "backdrop-filter", "perspective", "clip-path", "mask", "isolation"))), device), 
    // contain: strict | content | layout | paint
    hasComputedStyle("contain", (value) => {
        return Keyword.isKeyword(value)
            ? value.is("strict", "content")
            : value.layout || value.paint;
    }, device));
}
//# sourceMappingURL=creates-stacking-context.js.map
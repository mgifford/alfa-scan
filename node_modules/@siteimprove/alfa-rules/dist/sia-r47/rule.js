...
export default Rule.Atomic.of({
    ...
    evaluate({ document }) {
        let maximumScaleMap = Map.empty();
        let userScalableMap = Map.empty();
        return {
            applicability() {
                return (getElementDescendants(document)
                    .filter(and(hasNamespace(Namespace.HTML), hasName("meta"), hasAttribute("name", equals("viewport")), hasAttribute("content")))
            ...
            expectations(target) {
                return {
                    1: expectation(maximumScaleMap.get(target).every((scale) => scale >= 2) &&
                        userScalableMap
                            .get(target)
...
export var Outcomes;
(function (Outcomes) {
    Outcomes.MetaDoesNotPreventZoom = Ok.of(Diagnostic.of(`The \`<meta>\` element does not restrict the ability to zoom`));
    Outcomes.MetaDoesPreventZoom = Err.of(Diagnostic.of(`The \`<meta>\` element restricts the ability to zoom`));
})(Outcomes || (Outcomes = {}));
/**

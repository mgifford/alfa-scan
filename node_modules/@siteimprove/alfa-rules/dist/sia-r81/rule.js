...
const { hasNonEmptyAccessibleName, hasRole, isIncludedInTheAccessibilityTree } = DOM;
        ...
        return {
            applicability() {
                ...
                const map = getElementDescendants(document, dom.Node.fullTree)
                    .filter(and(hasNamespace(Namespace.HTML, Namespace.SVG), hasRole(device, (role) => role.is("link")), isIncludedInTheAccessibilityTree(device), hasNonEmptyAccessibleName(device)))
                    ...
                    .map((elements) => elements.groupBy((element) => Node.from(element, device).name.map((name) => String.normalize(name.value))));
                // Drop the context and name keys
                const groups = Sequence.from(Iterable.flatMap(map.values(), (map) => map.values()));
                // Only keep the groups with more than one element
                return groups.filter((links) => links.size > 1).map(Group.of);
            },
            expectations(target) {
                const name = WithName.getName(Iterable.first(target).getUnsafe(), // Existence of first element is guaranteed by applicability
                ...
                return {
                    1: expectation(embedSameResource, () => Outcomes.ResolveSameResource(name), () => Question.of("reference-equivalent-resources", target, `Do the links resolve to equivalent resources?`, {
                        diagnostic: WithName.of(`Do the links resolve to equivalent resources?`, name),
                    }).map((embedEquivalentResources) => expectation(embedEquivalentResources, () => Outcomes.ResolveEquivalentResource(name), () => Outcomes.ResolveDifferentResource(name)))),
                };
            },
        };
    },
...
export var Outcomes;
(function (Outcomes) {
    Outcomes.ResolveSameResource = (name) => Ok.of(WithName.of(`The links resolve to the same resource`, name));
    Outcomes.ResolveEquivalentResource = (name) => Ok.of(WithName.of(`The links resolve to equivalent resources`, name));
    Outcomes.ResolveDifferentResource = (name) => Err.of(WithName.of(`The links do not resolve to the same or equivalent resources`, name));
})(Outcomes || (Outcomes = {}));
/**

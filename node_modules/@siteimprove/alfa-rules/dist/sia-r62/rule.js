...
const { hasBorder, hasBoxShadow, hasComputedStyle, hasOutline, hasTextDecoration, isVisible, } = Style;
const { isText } = Text;
let distinguishingProperties = Map.empty();
        ...
        return {
            applicability() {
                ...
                function* getApplicableLinks() {
                    ...
                    linkColors.length === 1 &&
                        getForeground(container, device).some((containerColors) => 
            ...
            expectations(target) {
                ...
                const hasDistinguishingStyle = (context = Context.empty()) => Set.from(linkElements.map((link) => {
                    ...
                    const distinguishableContrast = Set.from(nonLinkElements.flatMap((container) => Sequence.from(Distinguishable.getPairwiseContrast(container, link, device, context))));
                    const properties = distinguishingProperties
                        .get(context)
                        .flatMap((elementMap) => elementMap.get(link))
                        .getOrElse(() => List.empty());
                    const diagnostic = ElementDistinguishable.from(link, device, target, context, properties, distinguishableContrast);
...
(function (Outcomes) {
    ...
    Outcomes.IsDistinguishable = (defaultStyles, hoverStyles, focusStyles) => Ok.of(DistinguishingStyles.of(`The link is distinguishable from the surrounding text`, defaultStyles, hoverStyles, focusStyles));
    Outcomes.IsNotDistinguishable = (defaultStyles, hoverStyles, focusStyles) => Err.of(DistinguishingStyles.of(`The link is not distinguishable from the surrounding text`, defaultStyles, hoverStyles, focusStyles));
})(Outcomes || (Outcomes = {}));
const hasNonLinkTextCache = Cache.empty();
function hasNonLinkText(device) {
    return function hasNonLinkText(element) {
        return hasNonLinkTextCache.get(element, () => {
            ...
            if (element.inclusiveAncestors(Node.flatTree).some(and(isElement, hasRole(device, (role) => role.is("link"))))) {
                return false;
            }
            const children = element.children(Node.flatTree);
            // If we've found text with more than whitespaces, we're done.
            if (children.some(and(isText, and(isVisible(device), (text) => !String.isWhitespace(text.data, false))))) {
                return true;
            }
            // Otherwise, go down.
            return (children
...
(function (Distinguishable) {
    ...
    function hasDistinguishableBackground(container, device, context) {
        ...
        return (link) => {
            ...
            const hasBackground = !(Keyword.isKeyword(image.values[0]) &&
    ...
    function getPairwiseContrast(container, link, device, context = Context.empty()) {
        return getForeground(container, device, context)
            .map((containerColors) => Array.flatMap(containerColors, (containerColor) => getForeground(link, device, context)
            .map((linkColors) => Array.map(linkColors, (linkColor) => Contrast.Pairing.of(["container", containerColor], ["link", linkColor], contrast(containerColor, linkColor))))
            .getOr([])))
            .getOr([]);
    }
    ...
    function hasDistinguishableFont(container, device, context) {
        ...
        return or(hasComputedStyle("font-weight", not((weight) => weight.equals(referenceWeight)), device, context), hasComputedStyle("font-family", not((family) => Option.from(family.values[0]).equals(referenceFamily)), device, context));
    }

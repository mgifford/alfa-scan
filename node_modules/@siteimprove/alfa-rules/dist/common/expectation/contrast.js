 ...
 * @internal
 */
export function hasSufficientContrast(target, device, largeTextThreshold, normalTextThreshold) {
    ...
    const parent = target.parent(Node.flatTree).getUnsafe();
    ...
    const background = getBackground(parent, device)
        .err()
        .map((errors) => flatMap(takeWhile(errors.errors, ColorError.isInterposedDescendants), (error) => error.positionedDescendants))
        .getOr([]);
    ...
    const ignoredInterposedElements = Question.of("ignored-interposed-elements", Group.of(interposedDescendants), target).answerIf(getIntersectors(parent, interposedDescendants, device).map((intersectors) => interposedDescendants.subtract(intersectors)));
    ...
    return {
        1: ignoredInterposedElements.map((ignored) => {
            ...
            return foregrounds
                ...
                .answerIf(getBackground(parent, device, undefined, undefined, ignoredInterposed))
                .map((backgrounds) => {
                const { pairings, highest } = getPairings(foregrounds, backgrounds);
                return expectation(
                // Accept if  single pairing is good enough.
                highest >= threshold, () => Outcomes.HasSufficientContrast(highest, threshold, pairings), () => Outcomes.HasInsufficientContrast(highest, threshold, pairings));
            }));
        }),
    };
}
...
function getPairings(foregrounds, backgrounds) {
    return cache.get(foregrounds, Cache.empty).get(backgrounds, () => {
        const pairings = [
            ...flatMap(foregrounds, (foreground) => map(backgrounds, (background) => Diagnostic.Pairing.of(["foreground", foreground], ["background", background], contrast(foreground, background)))),
        ];
...
function getIntersectors(element, candidates, device) {
    ...
    if (!elementBox.isSome() ||
        Iterable.some(candidates, (candidate) => candidate.getBoundingBox(device).isNone())) {
        return None;
    }
    return Option.of(Iterable.filter(candidates, (canditate) => elementBox
        .get()

 ...
 * @public
 */
export function getForeground(element, device, context = Context.empty(), ignoredInterposedDescendants = Set.empty()) {
    return foregroundCache
        ...
        if (backgroundColors.isOk()) {
            ...
            const colors = backgroundColors.map((background) => background.map((backdrop) => Color.composite(color, backdrop, 1)));
            ...
            for (const parent of element.parent(Node.flatTree).filter(isElement)) {
                return colors.flatMap((colors) => getBackground(parent, device, context, undefined, ignoredInterposedDescendants).map((background) => colors.flatMap((color) => background.map((backdrop) => Color.composite(color, backdrop, opacity.value)))));
            }

...
export default Rule.Atomic.of({
    ...
    evaluate({ device, document }) {
        return {
            applicability() {
                return getElementDescendants(document, Node.fullTree).filter(and(hasNamespace(Namespace.HTML), isVisible(device), isPossiblyScrollable(device), not(isBrowsingContextContainer)));
            },
            expectations(target) {
                return {
                    1: expectation(Node.hasInclusiveDescendant(and(isElement, isTabbable(device)), Node.flatTree)(target), () => Outcomes.IsReachable, () => Outcomes.IsNotReachable),
                };
            },
        };
    },
...
export var Outcomes;
(function (Outcomes) {
    Outcomes.IsReachable = Ok.of(Diagnostic.of(`The scrollable element is reachable through keyboard navigation`));
    Outcomes.IsNotReachable = Err.of(Diagnostic.of(`The scrollable element is not reachable through keyboard navigation`));
})(Outcomes || (Outcomes = {}));
/**

...
export default Rule.Atomic.of({
    ...
    evaluate({ device, document }) {
        return {
            applicability() {
                return getElementDescendants(document, Node.fullTree).filter(and(hasNamespace(Namespace.HTML), hasName("object"), isIncludedInTheAccessibilityTree(device), embedsMedia));
            },
            expectations(target) {
                return {
...
export var Outcomes;
(function (Outcomes) {
    Outcomes.HasName = Ok.of(Diagnostic.of(`The \`<object>\` element has an accessible name`));
    Outcomes.HasNoName = Err.of(Diagnostic.of(`The \`<object>\` element does not have an accessible name`));
})(Outcomes || (Outcomes = {}));

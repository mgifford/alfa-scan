...
import { Document, Element, MediaRule, Node, Query, Shadow, StyleRule, } from "@siteimprove/alfa-dom";
    ...
    evaluate({ device, document }) {
        ...
        return {
            applicability() {
                ...
                const orientationRules = getMediaRules(document).filter((rule) => some(rule.queries.queries, (query) => query.condition.some(hasOrientationCondition)));
                ...
                const rotationRulesDeclarations = orientationRules
                    ...
                    .collect((rule) => rule.style.declaration((declaration) => ["rotate", "transform"].includes(declaration.name)));
            ...
            expectations(target) {
                const rotation = getRelativeRotation(target, landscape, portrait).map((rotation) => Real.round(rotation));
                return {
                    1: expectation(rotation.every((rotation) => rotation !== 90 && rotation !== 270), () => Outcomes.RotationNotLocked, () => Outcomes.RotationLocked),
                };
            },
        };
    },
...
export var Outcomes;
(function (Outcomes) {
    Outcomes.RotationNotLocked = Ok.of(Diagnostic.of(`The element is not locked in orientation`));
    Outcomes.RotationLocked = Err.of(Diagnostic.of(`The element is locked in orientation`));
})(Outcomes || (Outcomes = {}));
function hasConditionalRotation(device) {
    ...
    // The only keyword value is "none", which is no rotation
    !Keyword.isKeyword(value) && source.some(isOrientationConditional), device));
}
function isOrientationConditional(declaration) {
    return some(declaration.ancestors(), (rule) => MediaRule.isMediaRule(rule) &&
...
function hasOrientationCondition(condition) {
    for (const feature of condition) {
        if (feature.name === "orientation" &&
            feature.value.some((value) => value.matches(Keyword.of("landscape")) ||
...
function getRelativeRotation(element, left, right) {
    return getRotation(element, left).flatMap((left) => getRotation(element, right).map((right) => Real.modulo(abs(left - right), 360)));
}
function getMediaRules(node) {
    return Sequence.from(flatMap(flatMap(roots(node), (document) => document.style), (sheet) => sheet.descendants())).filter(MediaRule.isMediaRule);
}
function* roots(node) {

...
export default Rule.Atomic.of({
    ...
    evaluate({ device, document }) {
        ...
        return {
            applicability() {
                headings = document.descendants(Node.fullTree).filter(and(isElement, and(hasNamespace(Namespace.HTML), isIncludedInTheAccessibilityTree(device), hasRole(device, "heading"), 
            ...
            expectations(target) {
                ...
                return {
                    1: expectation(Node.getNodesBetween(target, next, {
                        includeFirst: false,
                        ...
                        includeSecond: end,
                    }).some(and(isIncludedInTheAccessibilityTree(device), isContent(Node.fullTree), not(and(isText, (text) => text.data.trim() === "")))), () => Outcomes.hasContent(
                    // The link between end nad the type of next is lost by TS
                    end ? None : Some.of(next), currentLevel, nextLevel), () => Outcomes.hasNoContent(
                    // The link between end nad the type of next is lost by TS
                    end ? None : Some.of(next), currentLevel, nextLevel)),
                };
            },
        };
    },
...
export var Outcomes;
(function (Outcomes) {
    Outcomes.hasContent = (nextHeading, currentLevel, nextLevel) => Ok.of(WithOtherHeading.of("There is content between this heading and the next", nextHeading, currentLevel, nextLevel, "next"));
    Outcomes.hasNoContent = (nextHeading, currentLevel, nextLevel) => Err.of(WithOtherHeading.of("There is no content between this heading and the next", nextHeading, currentLevel, nextLevel, "next"));
})(Outcomes || (Outcomes = {}));

        ...
        return {
            applicability() {
                function* visit(node, lang) {
                    ...
                    if (lang.isSome()) {
                        const isVisibleText = and(isText, and(or(isVisible(device), isIncludedInTheAccessibilityTree(device)), (text) => !String.isWhitespace(text.data, false)));
                        const isElementWithAccessibleName = and(isElement, hasNonEmptyAccessibleName(device));
            ...
            expectations(target) {
                return {
                    1: expectation(Language.parse(target.value).isOk(), () => Outcomes.HasValidLanguage, () => Outcomes.HasNoValidLanguage),
                };
            },
        };
    },
...
export var Outcomes;
(function (Outcomes) {
    Outcomes.HasValidLanguage = Ok.of(Diagnostic.of(`The \`lang\` attribute has a valid primary language subtag`));
    Outcomes.HasNoValidLanguage = Err.of(Diagnostic.of(`The \`lang\` attribute does not have a valid primary language subtag`));
})(Outcomes || (Outcomes = {}));

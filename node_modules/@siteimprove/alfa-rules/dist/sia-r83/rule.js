...
import { Document, Element, MediaRule, Namespace, Node, Query, Text, } from "@siteimprove/alfa-dom";
...
const { hasBox } = Node;
const { isText } = Text;
const { getPositioningParent, hasCascadedStyle, hasComputedStyle, hasUsedStyle, isVisible, } = Style;
export default Rule.Atomic.of({
    ...
    evaluate({ device, document }) {
        return {
            applicability() {
                return document
                    .inclusiveDescendants(Node.Traversal.of(Node.Traversal.composed, Node.Traversal.flattened))
                ...
                function* visit(node, collect = false) {
                    ...
                    if (test(and(isElement, or(hasAttribute("aria-hidden", equals("true")), not(hasNamespace(Namespace.HTML)))), node)) {
                        return;
                    }
            ...
            expectations(target) {
                const horizontallyClippedBy = ClippingAncestor.horizontal(device, target);
                ...
                return {
                    ...
                    // false negatives.
                    expectation(horizontallyClippedBy.every(hasBig("width")) &&
                        verticallyClippedBy.every(hasBig("height")), () => Outcomes.IsContainer(horizontallyClippedBy, verticallyClippedBy), () => Outcomes.ClipsText(horizontallyClippedBy, verticallyClippedBy)), () => Outcomes.WrapsText),
                };
            },
        };
    },
 ...
 */
function textBox(text, device) {
    return text.getBoundingBox(device).orElse(() => text
        .parent(Node.fullTree)
        .filter(isElement)
        .flatMap((parent) => parent.getBoundingBox(device)));
}
const isWrappingFlexContainer = (device) => hasUsedStyle("flex-wrap", (value) => value.some((wrap) => !wrap.is("nowrap")), device);
/**
 ...
 */
function lineBox(text, device) {
    const isBlockContainer = (element) => Style.isBlockContainer(Style.from(element, device));
    const hasFlexWrapParent = (element) => element
        .parent(Node.fullTree)
        .filter(Element.isElement)
        .some(isWrappingFlexContainer(device));
    return Rectangle.union(...text
        .ancestors(Node.fullTree)
        .find(and(isElement, or(isBlockContainer, hasFlexWrapParent)))
        .map((lineContainer) => Query.getDescendants(isText)(lineContainer, Node.fullTree))
...
function isTwiceAsBig(target, device) {
    return (dimension) => hasBox((clippingBox) => clippingBox[dimension] >= 2 * lineBox(target, device)[dimension], device);
}
var ClippingAncestor;
(function (ClippingAncestor) {
    ClippingAncestor.vertical = clipper("height", localVerticalOverflow);
    // This is eta-expanded to avoid premature evaluation of the localHorizontalOverflow function.
    ClippingAncestor.horizontal = (device, text) => clipper("width", localHorizontalOverflow())(device, text);
        ...
        return (device, text) => {
            function clipper(element) {
                return clipperCaches[dimension]
                    ...
                    if (test(Media.usesFontRelativeMediaRule(device, predicates[dimension]), element)) {
     ...
     * an ancestor element.
     */
    function localVerticalOverflow(device, text, ancestor) {
        const verticalOverflow = overflow(ancestor, device, "y");
        switch (verticalOverflow) {
            case Overflow.Clip:
                return constrainingAncestor(ancestor, device, "height").every(isTwiceAsBig(text, device)("height"))
    ...
    function localHorizontalOverflow() {
        ...
        return (device, text, ancestor) => {
            ...
            if (inSameBlock &&
                test(and(hasComputedStyle("text-wrap-mode", (value) => value.is("wrap"), device), hasSoftWrapPoints(device)), ancestor)) {
            ...
            if (horizontalOverflow === Overflow.Clip) {
                if (inSameBlock &&
                    hasUsedStyle("text-overflow", (value) => value.some((overflow) => overflow.is("ellipsis")), device)(ancestor)) {
                ...
                if (constrainingAncestor(ancestor, device, "width").every(isTwiceAsBig(text, device)("width"))) {
    ...
    const _softWrapPointsCache = Cache.empty();
    /**
    ...
    function hasSoftWrapPoints(device) {
        return (node) => _softWrapPointsCache.get(device, Cache.empty).get(node, () => {
            if (isText(node)) {
            ...
            if (isElement(node)) {
                ...
                if (hasComputedStyle("text-wrap-mode", (value) => value.is("nowrap"), device)(node)) {
                    return false;
                }
    ...
    function constrainingAncestor(element, device, dimension) {
        return constrainingCaches[dimension]
            .get(device, Cache.empty)
            .get(element, () => hasFixedDimension(device, dimension)(element) ||
            ...
            ? Option.of(element)
            : getPositioningParent(element, device).flatMap((parent) => constrainingAncestor(parent, device, dimension)));
    }
    /**
    ...
    function hasFixedDimension(device, dimension) {
        return hasCascadedStyle(dimension, (value, source) => 
...
function hasFontRelativeValue(device, property) {
    return or(hasCascadedStyle(property, (value) => Length.isLength(value) &&
        (value.hasCalculation() || (value.value > 0 && value.isFontRelative())), device), hasCascadedStyle(`min-${property}`, (value) => Length.isLength(value) &&
...
(function (Media) {
    ...
    Media.usesFontRelativeMediaRule = usesFontRelativeMediaRule;
    const _mediaRulesCache = Cache.empty();
    ...
    const _ruleTreeCache = Cache.empty();
    ...
    function isFontRelativeMediaRule(predicate) {
        return (rule) => Iterable.some(rule.queries.queries, (query) => query.condition.some((condition) => Iterable.some(condition, (feature) => predicate(feature) &&
            feature.value.some((value) => Range.isRange(value)
                ? value.minimum.some((min) => Length.isLength(min.value) &&
...
export var Outcomes;
(function (Outcomes) {
    Outcomes.WrapsText = Ok.of(ClippingAncestors.of(`The text is wrapped without being clipped`));
    Outcomes.ClipsText = (horizontal, vertical) => Err.of(ClippingAncestors.of(`The text is clipped`, horizontal, vertical));
    Outcomes.IsContainer = (horizontal, vertical) => Ok.of(ClippingAncestors.of("The text would be clipped but the clipper is more than twice as large", horizontal, vertical));
})(Outcomes || (Outcomes = {}));

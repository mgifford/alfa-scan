...
export default Rule.Atomic.of({
    ...
    evaluate({ device, document }) {
        return {
            ...
            applicability() {
                let targets = [];
                const applicable = and(or(hasName("img"), hasName("canvas"), hasName("svg")), isVisible(device), hasAccessibleName(device));
                const namedFromAuthor = hasAccessibleName(device, (name) => name.source.some(({ type }) => type === "label" || type === "reference"));
                function visit(node) {
            ...
            expectations(target) {
                ...
                return {
                    1: Question.of("is-image-accessible-name-descriptive", target, message, {
                        diagnostic: WithName.of(message, accName),
                    }).map((descriptive) => expectation(descriptive, () => Outcomes.ImageAccessibleNameIsDescriptive(accName), () => Outcomes.ImageAccessibleNameIsNotDescriptive(accName))),
                };
            },
        };
    },
...
export var Outcomes;
(function (Outcomes) {
    Outcomes.ImageAccessibleNameIsDescriptive = (accName) => Ok.of(WithName.of("This accessible name describes the image", accName));
    Outcomes.ImageAccessibleNameIsNotDescriptive = (accName) => Err.of(WithName.of("This accessible name does not describe the image", accName));
})(Outcomes || (Outcomes = {}));
